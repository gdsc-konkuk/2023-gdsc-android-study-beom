...
class HomeFragment : Fragment() {
    ...
    private lateinit var adapter: ToDoAdaptor
    ...
    fun initialize(){
        with(todolist){
            add(ToDo(1, "안드로이드 스터디 과제하기", false))
            add(ToDo(2, "안드로이드 자기소개 구경하기", true))
            add(ToDo(3, "안드로이드 2주차 공부하기", true))
           add(ToDo(4, "안드로이드 1주차 복습하기", false))
            add(ToDo(5, "이슬톡톡 연마하기", true))
            add(ToDo(6, "살아남기..!", true))
        }
    }
}
